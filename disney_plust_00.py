...
# URL 설정
DISNEY_PLUS_URL = "https://www.justwatch.com/kr/%EB%8F%99%EC%98%81%EC%83%81%EC%84%9C%EB%B9%84%EC%8A%A4/disney-plus?release_year_from=2024&release_year_until=2024&rating_imdb=6&tomatoMeter=60"
...
df.to_csv("Disnry_plus_update.csv", index=False, encoding="utf-8-sig")

print(f"총 {len(df)}개의 작품 정보를 크롤링했습니다.")

# 데이터 클리닝
df = pd.read_csv("Disnry_plus_update.csv", encoding="utf-8-sig")
...
# 데이터 형식 변환
df['Rating IMDB'] = df['Rating IMDB'].str.extract(r'(\d+\.\d+)').astype(float, errors='ignore')
df['Rating TOMATO'] = df['Rating TOMATO'].str.extract(r'(\d+)%').astype(float, errors='ignore')
...
# 클리닝된 데이터 저장
df.to_csv("disney_plus_details_cleaned.csv", index=False, encoding="utf-8-sig")

print("데이터 클리닝 완료. 클리닝된 데이터가 'disney_plus_details_cleaned_v1.csv' 파일에 저장되었습니다.")from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.chrome.service import Service
from webdriver_manager.chrome import ChromeDriverManager
import pandas as pd
...
# 클리닝된 데이터 저장
df.to_csv("disney_plus_details_cleaned.csv_00", index=False, encoding="utf-8-sig")

print("데이터 클리닝 완료. 클리닝된 데이터가 'disney_plus_details_cleaned_v1.csv' 파일에 저장되었습니다.")
